    ...
    <el-col :span="24">
      <el-form :model="ruleForm" :rules="rules" ref="ruleForm" class="formwidth">
        ...
        <el-form-item
          ...
          :label-width="this.formLabelWidth"
          class='InpitWidth'>
          <el-input clearable autocomplete="off" placeholder="请输入服务端口号" v-model="ruleForm.serverPort"></el-input>
        </el-form-item>
        <el-form-item
          prop="remark"
          label="备注"
    ...
    async initial (sid) {
      ...
      try {
        ...
        if (response.data.code === 1) {
          ...
          that.ruleForm.type = response.data.data.type
          that.ruleForm.createTime = response.data.data.createTime
          that.ruleForm.name = response.data.data.name
          that.ruleForm.url = response.data.data.url
          that.ruleForm.remark = response.data.data.remark
          that.ruleForm.serverPort = response.data.data['server.port']
    ...
    async edit (data) {
      ...
      try {
        ...
        if (response.data.code === 1) {
          ...
          that.$emit('saveUrl', that.ruleForm.url)
        } else {
          // 添加上传失败后 回调失败信息
