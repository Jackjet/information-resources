...
export default {
  ...
  methods: {
    ...
    async askInfo () {
      const that = this
      try {
        let findUrl = this.Interface.emqBuildRecord.findAll
    ...
    async toPack () {
      ...
      that.$confirm('是否打包?', '提示', {
        ...
        let url = that.Interface.emqBuildRecord.insert
    ...
    delete (row) {
      ...
      that.$confirm('此操作将永久删除数据, 是否继续?', '提示', {
        ...
        let url = this.Interface.emqBuildRecord.deleteById
        that.Loading = true
    ...
    async save () {
      const that = this
      let url = that.Interface.emqBuildRecord.updateById
      // POST请求添加轮播图数据
      that.Loading = true
