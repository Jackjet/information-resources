<template>
  <el-main>
    <el-col :span="24">
      <el-form :inline="true" class='el-InputForm'>
        <el-form-item style='margin-left: 15px;'>
          <el-button size='medium' @click="toBack" style="color: #5677DF" icon="el-icon-back">返回</el-button>
          <el-button size='medium' @click="showDialog" style="color: #5677DF;margin-left: 15px;" icon="el-icon-upload">导入作业</el-button>
          <el-button size='medium' @click="debugDialog" style="color: #5677DF;margin-left: 15px;" icon="el-icon-caret-right">调试</el-button>
          <el-button size='medium' @click="downloadFiles" style="color: #5677DF;margin-left: 15px;" icon="el-icon-download">下载</el-button>
          <el-button size='medium' @click="deleteFiles" style="color: #5677DF;margin-left: 15px;" icon="el-icon-delete">删除</el-button>
        </el-form-item>
      </el-form>
    </el-col>
    ...
    <el-col :span="24" class="center">
      <el-card class="box-card" style="margin-bottom: 20px; text-align: left;">
        <div slot="header" class="clearfix">
          <span>日志</span>
...
<script>
// import { findByTaskIdAndType, downloadTaskKettleFiles, deleteTaskKettleFiles, runTask } from "@/api/task.js";
...
export default {
  ...
  data() {
    return {
      ...
      action: this.PortUrl  + '/webadmin/task/kettleFiles/uploadFile',
      taskGroupId: '',
      nodeImg: {
  ...
  methods: {
    ...
    async fetchData() {
      const that = this
      try {
        let url = that.Interface.kettleFiles.findByType
        let res = await that.request.dataGet(that, url, {taskId: this.$route.query.id,type:'kjb'})
        if (res.data.code === 1) {
    ...
    async getContainer(){
      const that = this
      let url = that.Interface.container.findById;
      let res = await that.request.dataGet(that, url, {id: that.containerId})
      if (res.data.code === 1) {
        that.container= res.data.data
              ...
              ),
              {
                doubleClick: function (e, node) {
                  ...
                  if (node.part.data.path != null) {
                    location.href = "/#/taskManage/trans/transDetail?xml=" + node.part.data.path;
                  }
                },
    ...
    async downloadFiles() {
      const that = this
      let url = that.Interface.kettleFiles.download
      let response = await that.request.dataGet(that, url, {taskId: this.taskId})
      // const response = await downloadTaskKettleFiles(that.taskId)
      if (response.data.code === 1) {
        let url = this.PortUrl + response.data.data
    ...
    async deleteFiles() {
      const that = this
      let url = that.Interface.kettleFiles.delete
      let response = await that.request.dataDelete(that, url, {taskId: that.taskId})
      if (response.data.code === 1) {
        that.$message.success("删除成功")
        this.myDiagram.model=window.go.Model.fromJson({})
        this.fetchData()
      } else {
        that.$message.error(response.data.msg)
      }
    ...
    async debugDialog(){
      ...
      let url = this.Interface.task.run
      let response = await this.request.dataGet(this, url, {id: this.taskId})
      if (response.data.code === 1) {
