    ...
    <dialog-form :isShow="isShow" :title="title" @closeDialog="closeDialog" @saveDialog="saveDialog">
      <el-form slot="form" ref="form" :model="form" :rules="rules" label-width="120px">
        <el-form-item label="租户名称" prop="name">
          <el-input autocomplete="off" :disabled="id ? true : false" v-model="form.name"></el-input>
        </el-form-item>
        <el-form-item label="用户数量">
          <el-input autocomplete="off" v-model="form.integrationProjectAmount"></el-input>
        </el-form-item>
  ...
  methods: {
    ...
    saveDialog () {
      ...
      this.$refs['form'].validate(async (valid) => {
        if (valid) {
          try {
            let url = that.Interface.tenants.insert
            let obj = {
              name: that.form.name,
              integrationProjectAmount: that.form.integrationProjectAmount,
              remark: that.form.remark
            }
            let response = null
            if (that.id) {
              url = that.Interface.tenants.update
              obj.id = that.id
    ...
    async edit (row) {
      const that = this
      try {
        let findUrl = that.Interface.tenants.findById
    ...
    async getList () {
      const that = this
      try {
        let findUrl = this.Interface.tenants.findAll
    ...
    resetPassword (row) {
      ...
      that.$confirm('是否重置?', '提示', {
        ...
        let url = this.Interface.tenants.resetPassword + '?id=' + row.id
