        ...
        <el-row>
          ...
          <el-col :span="24">
            <el-table :data="tableData" stripe empty-text="暂无数据" class="el_tab_alage">
              ...
              <el-table-column align="left" fit prop="updateTime" label="数据挂接时间" width="110px">
                <template slot-scope="scope">
              ...
              <el-table-column align="left" fit label="操作" width="240px">
                <template slot-scope="scope">
                  <el-button size="mini" type="text" icon="el-icon-edit" v-if="permissions.edit" @click="handleEdit(scope.row)">编排
                  </el-button>
                  <el-button size="mini" type="text" icon="el-icon-paperclip" v-if="permissions.sync && scope.row.status === '已部署'" slot="reference" @click="handleSync(scope.row)">同步
                  </el-button>
                  <el-button size="mini" type="text" icon="el-icon-paperclip" v-if="permissions.test && scope.row.status !== '待编排'" slot="reference" @click="testApiDesign(scope.row)">测试
                  </el-button>
                </template>
              </el-table-column>
            </el-table>
...
<script>
import { findAll, getTree, insertApiGroup, updateApiGroup, deleteApiGroupById } from "@/api/apiDesign";
import { apiDesignInsert } from "@/api/resources"
...
export default {
  ...
  methods: {
    ...
    async handleSync(row) {
      ...
      console.log(row)
      try {
        const res = await apiDesignInsert(row.id)
        that.isSubmitLoading = false
        if (res.data.code === 1) {
