...
public interface ApiTestLogDao extends JpaRepository<ApiTestLog, String>, JpaSpecificationExecutor<ApiTestLog> {
     ...
     * @return 查询结果
     */
    @Query(value = "SELECT NEW d1.project.d1project.apidesign.model.ApiLogVm(info.id,info.method,info.requestTime,info.responseCode,info.responseTime) FROM ApiTestLog AS info WHERE info.apiId = ?1 AND info.requestTime BETWEEN ?2 AND ?3 ORDER BY info.requestTime DESC")
