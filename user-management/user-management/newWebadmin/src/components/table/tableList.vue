  ...
  <el-table :data='tableData' stripe @expand-change="expandSelect" :expand-row-keys="expands" :header-cell-style="headerStyle" :cell-style="cellStyle" empty-text='暂无数据' class='el_tab_alage' :row-key='getRowKeys' @selection-change="handleSelectionChange">
      ...
      <el-table-column fit align='center' :key='item' :sortable='index.sort' v-if="index.type !== 'expand'" :label="index.label" :width="index.width" :show-overflow-tooltip="false" :prop="index.list">
        <template slot-scope="scope">
          ...
          <template v-else-if="index.type === 'color2' && scope.row.status !== 1">
            <div class="center">
              <svg t="1601279073578" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="3588" width="20" height="20">
                <path d="M499.2 92.16l-409.6 230.4v486.4c0 56.32 46.08 102.4 102.4 102.4h614.4c56.32 0 102.4-46.08 102.4-102.4v-486.4l-409.6-230.4z m-358.4 716.8V407.04l232.96 184.32-230.4 230.4c-2.56-2.56-2.56-7.68-2.56-12.8z m40.96 48.64l235.52-235.52 84.48 56.32 84.48-56.32 235.52 235.52h-640z m675.84-48.64c0 5.12 0 10.24-2.56 15.36l-230.4-230.4 232.96-184.32v399.36z m-358.4-189.44l-358.4-273.92 358.4-197.12 358.4 197.12-358.4 273.92z" p-id="3589" fill="#ebb563"></path>
              </svg>
          ...
          <template v-else-if="index.type === 'color2' && scope.row.status == 1">
            <div class="center">
              <svg t="1601279073578" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="3588" width="20" height="20">
                <path d="M499.2 92.16l-409.6 230.4v486.4c0 56.32 46.08 102.4 102.4 102.4h614.4c56.32 0 102.4-46.08 102.4-102.4v-486.4l-409.6-230.4z m-358.4 716.8V407.04l232.96 184.32-230.4 230.4c-2.56-2.56-2.56-7.68-2.56-12.8z m40.96 48.64l235.52-235.52 84.48 56.32 84.48-56.32 235.52 235.52h-640z m675.84-48.64c0 5.12 0 10.24-2.56 15.36l-230.4-230.4 232.96-184.32v399.36z m-358.4-189.44l-358.4-273.92 358.4-197.12 358.4 197.12-358.4 273.92z" p-id="3589" fill="#a2a2a2"></path>
              </svg>
    ...
    <el-table-column fit align='center' :label="tableOption.label" :width="tableOption.width" v-if="tableOption.value === 0">
      <template style="margin-left: 30px;" slot-scope="scope">
        <el-button align='right' v-for="(value, item) in tableOption.options" :key='item' v-if="value.show ? value.show(scope.row) : true" :type="value.type" :style="value.style ? JSON.parse(value.style) : {}" :plain='value.State' :icon="value.icon" @click.native.prevent="value.method(scope.row, scope)">{{value.label}}
        </el-button>
      </template>
    </el-table-column>
  </el-table>
...
export default {
  ...
  mounted() {
    console.log(this.tableData, "00000000000")
  },
  methods: {
    handleHref(row) {
