...
<script>
import { menuTreeFindBySystem, menuTreeUpdateSeq, menuTreeDeleteAll, menuTreedelete } from "@/api/menu.js"
...
export default {
  data() {
    return {
      ...
      DeletelistiD: [],
      systemCodeList:[
        {label:'用户管理系统',value:0},
        {label:'资源管理系统',value:1},
        {label:'API编排系统',value:2},
        {label:'API管理服务系统',value:3},
        {label:'数据集成服务系统',value:4},
        {label:'消息集成服务系统',value:5},
      ],
      systemCode:''
    }
  },
  created() {
    ...
    let code = this.$route.query.systemCode
    if (code){
      this.systemCode = Number(code)
    }else {
      this.systemCode = 0
    }
  ...
  methods: {
    ...
    async systemCodeChange(){
        this.fetchData()
    },

    async fetchData() {
      const data = {systemCode:this.systemCode}
      const that = this
      try {
        that.isSubmitLoading = true
        const res = await menuTreeFindBySystem(data)
