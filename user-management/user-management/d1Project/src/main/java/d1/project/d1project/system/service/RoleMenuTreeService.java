...
public class RoleMenuTreeService {
     ...
     */
    public List<MenuTreeTree> findAllByRoleId(Integer systemCode,String roleId, boolean isSidebar) {
        Set<String> collect;
        ...
        return  menuTreeService.findAllByIdIn(collect, isSidebar,systemCode);
    }
     ...
     */
    public List<String> findAllByRoleIdInButton(List<String> rolesIds,Integer systemCode) {
        ...
        if (rolesIds.contains("admin")) {
            ...
            allByIdInOrderBySeqAsc = menuTreeService.findAllByIdInAndSystemCodeOrderBySeqAsc(collect,systemCode);
        }

        //返回有权限的菜单数组
        return allByIdInOrderBySeqAsc.stream().filter(t->2==t.getType()).map(MenuTreeEntity::getPath).collect(Collectors.toList());
    }
     ...
     * @param roleId 角色id
     */
    public List<String> findAllRoleIdChoose(String roleId,Integer systemCode) {
        if ("admin".equals(roleId)) {
        ...
        return menuTreeService.findAllNoChildrenAsLongAsId(collect,systemCode);
    }
