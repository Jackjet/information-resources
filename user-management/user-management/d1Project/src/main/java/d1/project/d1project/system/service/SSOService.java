...
public class SSOService {
    ...
    public void updatePassword(String ssoId ,String password) throws DoValidException {
        try {
            if (StringUtils.isEmpty(ssoId)){
                String temp = authzUserService.findAll("","","","",0,1);
                List<UserRepresentation> list = JSONArray.parseArray(temp,UserRepresentation.class);
                UserRepresentation tempUser = list.get(0);
                ssoId = tempUser.getId();
            }
            authzUserService.put(ssoId, "", "", password);
    ...
    public void resetPassword(WebAdminUserEntity webAdminUserEntity) throws DoValidException {
        if (!StringUtils.isEmpty(webAdminUserEntity.getPassword())) {
            try {
                authzUserService.put(webAdminUserEntity.getSsoId(), "", webAdminUserEntity.getName(), "123456");
            } catch (Exception e) {
                throw new DoValidException(e.getMessage());
            }
        }
