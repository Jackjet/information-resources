...
import d1.project.d1project.common.utils.DataDictionary;
import d1.project.d1project.system.dao.MenuTreeDao;
import d1.project.d1project.system.dao.RoleMenuTreeDao;
import d1.project.d1project.system.entity.MenuTreeEntity;
import d1.project.d1project.system.mapper.MenuTreeMapper;
import d1.project.d1project.system.model.MenuTreeAll;
import d1.project.d1project.system.model.MenuTreeTree;
import d1.project.d1project.system.model.vm.MenuTreeInsertVm;
import d1.project.d1project.system.model.vm.MenuTreeUpdateSeqVm;
import d1.project.d1project.system.model.vm.MenuTreeUpdateVm;
...
public class MenuTreeService {
    ...
    public void insert(MenuTreeInsertVm model, HttpServletRequest request) throws DoValidException {
        ...
        if (menuTreeDao.existsByNameAndParentIdAndSystemCode(menuTree.getName(), parentId,menuTree.getSystemCode())) {
    ...
    public void update(MenuTreeUpdateVm model, HttpServletRequest request) throws DoValidException {
        ...
        if (menuTreeDao.existsByNameAndParentIdAndSystemCodeAndIdNot(model.getName(), parentId1,model.getSystemCode(), menuTree.getId())) {
            throw new DoValidException("请重新输入，该菜单名称同层级下已存在");
        }
     ...
     * 根据系统查询菜单信息
     */
    public List<MenuTreeTree> findAllFormatIntoVm(Integer systemCode) {
        List<MenuTreeEntity> seq = menuTreeDao.findAllBySystemCodeOrderBySeqAsc(systemCode);
     ...
     * 查询所有
     */
    public List<MenuTreeTree> findAllFormatVm() {
        List<MenuTreeEntity> seq = menuTreeDao.findAll(Sort.by(Sort.Direction.ASC, "seq"));
        List<MenuTreeTree> childList = getMenuTreeTreeVm(seq, false);
        Map<Integer,MenuTreeTree> map = new HashMap<>();
        int i = 0;
        for (MenuTreeTree menuTreeTree : childList) {
            int code = menuTreeTree.getSystemCode();
            if (i == 0){
                createNewTree(menuTreeTree,code,map);
            }else {
                MenuTreeTree tempTree = map.get(code);
                if (tempTree == null){
                    createNewTree(menuTreeTree,code,map);
                }else {
                    List<MenuTreeTree> tempList = tempTree.getChildren();
                    menuTreeTree.setParentId(Integer.toString(code));
                    tempList.add(menuTreeTree);
                    tempTree.setChildren(tempList);
                    map.put(code,tempTree);
                }
            }
            i++;
        }
        List<MenuTreeTree> returnList = new ArrayList<>();
        for (Map.Entry<Integer, MenuTreeTree> im : map.entrySet()) {
            returnList.add(im.getValue());
        }
        return returnList;
    }
     ...
     * 找到所有没有子集的菜单ID
     */
    public List<String> findAllNoChildrenAsLongAsId(List<String> collect,Integer systemCode) {
        List<String> menuTreeIds = new ArrayList<>();
        List<MenuTreeTree> allByIdIn = findAllByIdIn(collect, false,systemCode);
     ...
     */
    public List<MenuTreeTree> findAllByIdIn(Collection<String> id, boolean isSidebar,Integer systemCode) {
        ...
        if (systemCode == -1){
            ...
            if (isSidebar) {
                seq = menuTreeDao.findAllByIdInAndTypeInAndSystemCodeOrderBySeqAsc(id, Arrays.asList(0, 1),systemCode);
            } else {
                seq = menuTreeDao.findAllByIdInAndSystemCodeOrderBySeqAsc(id,systemCode);
            }
        }
        return getMenuTreeTreeVm(seq, isSidebar);
    }

    public List<MenuTreeEntity> findAllByIdInAndSystemCodeOrderBySeqAsc(Collection<String> id,Integer systemCode) {
        return menuTreeDao.findAllByIdInAndSystemCodeOrderBySeqAsc(id,systemCode);
    }
    ...
    private void createNewTree(MenuTreeTree menuTreeTree,int code,Map<Integer,MenuTreeTree> map){
        MenuTreeTree mtt = new MenuTreeTree();
        String parentId = menuTreeTree.getParentId();
        if (StringUtils.isEmpty(parentId)){
            menuTreeTree.setParentId(Integer.toString(code));
        }
        List<MenuTreeTree> list = new ArrayList<>();
        mtt.setId(Integer.toString(code));
        list.add(menuTreeTree);
        mtt.setChildren(list);
        mtt.setName(DataDictionary.SYS_NAME.get(code));
        mtt.setSystemCode(code);
        map.put(menuTreeTree.getSystemCode(),mtt);
    }
