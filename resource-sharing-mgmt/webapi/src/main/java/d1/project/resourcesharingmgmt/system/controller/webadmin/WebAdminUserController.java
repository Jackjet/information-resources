...
import d1.project.resourcesharingmgmt.system.service.WebAdminUserUploadDataListener;
...
//    }
    ...
    public Result<String> export(WebAdminUserFindAllVm model, HttpServletResponse response) throws Exception {
        List<WebAdminUserExcelBase> allList = webAdminUserService.findAllList(model);
        // 设置需要下拉的列样式
        CellWriteHandler cellWriteHandler = webAdminUserService.getCellWriteHandler(allList.size());
        // 这里注意 有同学反应使用swagger 会导致各种问题，请直接用浏览器或者用postman
        webAdminUserService.easyExcelWrite(response, allList, cellWriteHandler);
        return null;
    }
    ...
    public Result<String> export(@Valid @RequestBody IdsBaseVm model, HttpServletResponse response) throws Exception {
        List<WebAdminUserExcelBase> allList = webAdminUserService.findAllByIds(model.getIds());
        // 这里注意 有同学反应使用swagger 会导致各种问题，请直接用浏览器或者用postman
    ...
    public Result<String> exportModel(HttpServletResponse response) throws Exception {
        ...
        ids.add("admin");
        List<WebAdminUserExcelBase> allList = webAdminUserService.findAllByIds(ids);
        // 设置需要下拉的列样式
        CellWriteHandler cellWriteHandler = webAdminUserService.getCellWriteHandler(allList.size());
        // 这里注意 有同学反应使用swagger 会导致各种问题，请直接用浏览器或者用postman
        webAdminUserService.easyExcelWrite(response, allList, cellWriteHandler);
