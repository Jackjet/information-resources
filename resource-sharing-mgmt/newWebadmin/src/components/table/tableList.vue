<template>
  ...
  <el-table :data='tableData' stripe @expand-change="expandSelect" :expand-row-keys="expands" :header-cell-style="headerStyle" :cell-style="cellStyle" empty-text='暂无数据' class='el_tab_alage' :row-key='getRowKeys' @selection-change="handleSelectionChange">
    ...
    <template v-for="(index, item) in tableLabel">
      <el-table-column fit align='center' :key='item' :sortable='index.sort' v-if="index.type !== 'expand'" :label="index.label" :width="index.width" :show-overflow-tooltip="true" effect="light" :prop="index.list">
        <template slot-scope="scope">
          <!-- 图片 -->
          ...
          <template v-else-if="index.type === 'html'">
            <div v-html="index.code(scope.row)" style="overflow: hidden;white-space: nowrap;text-overflow: ellipsis;"></div>
          </template>
          <!-- 正常显示 -->
          <template v-else>
