<template>
  <el-autocomplete class="inline-input" v-model="inputValue" :size="size" clearable :trigger-on-focus="onFocus" :fetch-suggestions="querySearch" :placeholder="thePlaceholder"></el-autocomplete>
</template>
<script>
...
export default {
  ...
  computed: {
    theId() {
      this.dataTheId = this.$store.state.user.theId;
    }
  },
  methods: {
    ...
    querySearch(queryString, cb) {
      if (this.onFocus) {
        let local = []
        if (this.theId !== '' && this.dataTheId) {
          local = JSON.parse(this.dataTheId)
        }
      ...
      if (this.onFocus) {
        ...
        if (local) {
          ...
          if (flag) {
            ...
            this.$store.dispatch('setTheId', JSON.stringify(localArr))
          ...
          this.$store.dispatch('setTheId', JSON.stringify([{ 'value': this.inputValue }]))
        }
      }
    }
  }
