    ...
    <el-col class="main-center">
      <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="150px" class="demo-ruleForm">
        ...
        <el-form-item label="需求类型:" :label-width="this.formLabelWidth">
          <el-input maxlength='100' placeholder="" clearable size="medium" :disabled="true" v-model="requestType"></el-input>
          <!-- <el-select clearable v-model="ruleForm.requestType" placeholder="" :disabled="true">
        ...
        <el-form-item label="受理状态:" :label-width="this.formLabelWidth" prop='status'>
          <el-select clearable v-model="status" placeholder="请选择受理状态">
            <el-option key="1" label="已受理" value="1"></el-option>
            <el-option key="2" label="已驳回" value="2"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="受理意见:" :label-width="this.formLabelWidth">
          <el-input :rows='4' type="textarea" placeholder="请输入受理意见" clearable size="medium" v-model="opinion" style="width: 400px"></el-input>
        </el-form-item>
        <div class="demo-drawer__footer drawer_footer">
          <el-button size="medium" @click="resetForm('ruleForm')">取 消</el-button>
          <el-button size="medium" type="primary" v-if="!disabled" @click="submitForm('ruleForm')">确 认</el-button>
        </div>
      </el-form>
    </el-col>
  </el-main>
...
<script>
import { demandedInfoUpdate, demandedInfoFind } from "@/api/demandedInfo.js";
export default {
  data() {
    return {
      title: '受理',
  ...
  methods: {
    ...
    async demandedInfoDetail() {
      ...
      if (response.data.code === 1) {
        ...
        switch (response.data.data.requestType) {
          case "0":
            this.requestType = "目录资源变更";
            break;
          case "1":
            this.requestType = "目录资源新增";
            break
          case "2":
            this.requestType = "云数据变更";
            break;
          case "3":
            this.requestType = "云数据新增";
            break;
          default:
            this.requestType = "未知";
            break;
    ...
    submitForm(ruleForm) {
      this.$refs["ruleForm"].validate((valid) => {
        if (valid) {
          ...
          // 编辑
          demandedInfoUpdate(data).then((res) => {
            if (res.data.code === 1) {
              if (data.status === "1") {
