...
<script>
import { assetExFindAllByExamine, organizationFindAllById, assetExSubmit, assetExExamine } from "@/api/fileData.js";
// import {archBusiUviewExFind}
  ...
  methods: {
    ...
    treeDataListFun() {
      ...
      organizationFindAllById({ id: userInfo.organizations[0] }).then((res) => {
        ...
        if (res.data.code === 1) {
          ...
          //     i = index;
          //   }
          //   res.data.data[index].typNm = item.name;
          //   res.data.data[index].typId = item.idLink;
          // });
          // res.data.data.splice(i, 1);
          this.treeData = [res.data.data];
          // this.key = [res.data.data[0].idLink]
          // this.SearchItem.provOrgId = userInfo.id == 'admin' ? '' : userInfo.organizations[0]
          this.fetchData();
    ...
    async fetchData() {
      ...
      try {
        ...
        const res = await assetExFindAllByExamine(data);
    ...
    // 提交审核
    submitToExamine(data, status) {
      let text = data.type == 0 ? '文件' : '数据库';
      if (status === 0) {
        this.$prompt('此操作审核通过该' + text + ', 是否继续?', '审核通过', {
          inputType: 'textarea',
