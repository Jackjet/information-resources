  ...
  <el-main class="main">
    <el-col :span="24" style="padding-bottom: 20px; background-color: #fff;">
      <el-table v-loading="isSubmitLoading" class="table-list" :cell-style="{'font-size': '14px','height': '40px', 'padding': '0'}" :header-cell-style="{'font-size': '14px', 'height': '40px', 'padding': '0', 'background-color': '#F5F7FA'}" :data="tableData" style="width: 100%" row-key="id" stripe="true" indent="16" default-expand-all='true'>
        ...
        <el-table-column prop="levelMsg" label="层级" align="center" width="100px"></el-table-column>
        <el-table-column prop="seq" align="center" label="排序" width="100px">
        ...
        <!-- <el-table-column align="center" label="操作" width="200px">
          <template slot-scope="scope">
            <el-button type="text" size="medium" icon="el-icon-plus" v-if="permissions.add" @click="handleAdd(scope.row)">新增</el-button>
            <el-button type="text" size="medium" icon="el-icon-edit-outline" v-if="permissions.edit" @click="handleEdit(scope.row)">编辑</el-button>
            <el-button size="medium" type="text" icon="el-icon-delete" v-if="scope.row.levelMsg !== '一级' && permissions.delete" @click="handleDelete(scope.row)">删除</el-button>
          </template>
        </el-table-column> -->
      </el-table>
    </el-col>
