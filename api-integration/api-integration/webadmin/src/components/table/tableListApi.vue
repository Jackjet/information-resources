      ...
      <el-table-column fit
        ...
        :prop="index.list">
         <template slot-scope="scope">
          ...
          <template v-else-if="index.type === 'color1' && scope.row.status === 1">
            <div class="center">
             <svg t="1601279073578" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="3588" width="20" height="20"><path d="M499.2 92.16l-409.6 230.4v486.4c0 56.32 46.08 102.4 102.4 102.4h614.4c56.32 0 102.4-46.08 102.4-102.4v-486.4l-409.6-230.4z m-358.4 716.8V407.04l232.96 184.32-230.4 230.4c-2.56-2.56-2.56-7.68-2.56-12.8z m40.96 48.64l235.52-235.52 84.48 56.32 84.48-56.32 235.52 235.52h-640z m675.84-48.64c0 5.12 0 10.24-2.56 15.36l-230.4-230.4 232.96-184.32v399.36z m-358.4-189.44l-358.4-273.92 358.4-197.12 358.4 197.12-358.4 273.92z" p-id="3589" fill="#e9bd4f"></path></svg>
              <span style="cursor: pointer; padding-left: 3px;" >{{scope.row[index.list]}}</span>
            </div>
          </template>
         ...
         <template v-else-if="index.type === 'imgColor'">
      ...
      <template slot-scope="scope">
        <template v-for="(value, item) in tableOption.options">
          <el-button
            ...
            v-if="scope.row.id!== 'admin' && item === 2 && scope.row.enable === false"
            @click.native.prevent="value.method(scope.row, scope)">启用
          </el-button>
          <el-button
            :type="value.type"
            :plain='value.State'
            ...
            :key='item'
            v-permission="value.permission"
            v-else-if="scope.row.id!== 'admin' && item === 2 && scope.row.enable === true "
            @click.native.prevent="value.method(scope.row, scope)">禁用
          </el-button>
          <template v-else>
            <el-button
              :type="value.type"
              ...
              :key='item'
              v-permission="value.permission"
              v-if="item === 0 || item === 2 && scope.row.id !== 'admin' || item === 1 && scope.row.id !== 'admin'"
              @click.native.prevent="value.method(scope.row, scope)">{{value.label}}
            </el-button>
          </template>
      </template>
      ...
      <template slot-scope="scope">
        <template v-for="(value, item) in tableOption.options">
            <el-button
              ...
              :icon="value.icon"
              :key='item'
              v-if="item === 0 || item === 1 || item === 2 && scope.row.id !== 'admin' && scope.row.id !== 'default' || item === 3 && scope.row.id !== 'admin' && scope.row.id !== 'default'"
              @click.native.prevent="value.method(scope.row, scope)">{{value.label}}
            </el-button>
          </template>
     </template>
