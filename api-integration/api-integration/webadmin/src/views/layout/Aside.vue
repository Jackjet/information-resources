...
export default {
  ...
  methods: {
    async initRole(id) {
      let buttons = await findCurrentUserButton({userId: id,systemCode:3});
      if (buttons.data.code === 1) {
        sessionStorage.setItem("UserButtons", JSON.stringify(buttons.data.data))
      }
      let menus = await findCurrentUserSidebar({userId: id,systemCode: 3})
      if (menus.data.code === 1) {
        this.items = menus.data.data[0].menuTree
