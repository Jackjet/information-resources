...
module.controller('UserFederatedIdentityCtrl', function($scope, $location, realm, user, federatedIdentities, UserFederatedIdentity, Notifications, Dialog) {
    ...
    $scope.removeProviderLink = function(providerLink) {
        ...
        Dialog.confirmDelete(providerLink.identityProvider, '标识提供程序链接', function() {
...
module.controller('UserListCtrl', function($scope, realm, User, UserSearchState, UserImpersonation, BruteForce, Notifications, $route, Dialog) {
    ...
    $scope.removeUser = function(user) {
        Dialog.confirmDelete(user.id, '用户', function() {
            user.$remove({
                realm : realm.realm,
                userId : user.id
            }, function() {
...
module.controller('UserTabCtrl', function($scope, $location, Dialog, Notifications, Current) {
    $scope.removeUser = function() {
        Dialog.confirmDelete($scope.user.id, '用户', function() {
            $scope.user.$remove({
...
module.controller('UserCredentialsCtrl', function($scope, realm, user, $route, $location, RequiredActions, User, UserExecuteActionsEmail,
    ...
    $scope.updateCredentialLabel = function(credential) {
        UserCredentials.updateCredentialLabel({ realm: realm.realm, userId: user.id, credentialId: credential.id }, {
            ...
            Notifications.error("操作失败。有关详细信息，请参阅控制台");
            console.log(err);
        });
    }

    $scope.deleteCredential = function(credential) {
        Dialog.confirm('删除凭据', '是否确实要删除这些用户凭据?', function() {
    ...
    $scope.resetPassword = function() {
        ...
        if ($scope.pwdChange) {
            if ($scope.password != $scope.confirmPassword) {
                Notifications.error("两次密码不匹配");
                return;
            }
        }

        var msgTitle = ($scope.hasPassword ? '重置' : '设置') + '密码';
        var msg = '确定是否要' + ($scope.hasPassword ? '重置' : '设置') + '密码?';
        var msgSuccess = '密码已经' + ($scope.hasPassword ? '重置' : '设置');
    ...
    $scope.sendExecuteActionsEmail = function() {
        if ($scope.changed) {
            Dialog.message("无法发送电子邮件", "必须先保存当前更改，然后才能发送电子邮件");
            return;
        }
        Dialog.confirm('发送电子邮件', '您确定要向用户发送电子邮件吗?', function() {
            UserExecuteActionsEmail.update({ realm: realm.realm, userId: user.id, lifespan: $scope.emailActionsTimeout.toSeconds() }, $scope.emailActions, function() {
                Notifications.success("电子邮件发送成功");
            }, function() {
                Notifications.error("电子邮件发送失败");
            });
        });
    };
...
module.controller('UserFederationCtrl', function($scope, $location, $route, realm, serverInfo, Components, Notifications, Dialog) {
    ...
    $scope.removeInstance = function(instance) {
        Dialog.confirmDelete(instance.name, '用户存储提供程序', function() {
            Components.remove({
                realm : realm.realm,
...
module.controller('GenericUserStorageCtrl', function($scope, $location, Notifications, $route, Dialog, realm,
    ...
    function triggerSync(action) {
        UserStorageOperations.sync.save({ action: action, realm: $scope.realm.realm, componentId: $scope.instance.id }, {}, function(syncResult) {
            $route.reload();
            Notifications.success("操作成功. " + syncResult.status);
        }, function() {
            $route.reload();
            Notifications.error("操作失败");
        });
...
module.controller('UserGroupMembershipCtrl', function($scope, $q, realm, user, UserGroupMembership, UserGroupMembershipCount, UserGroupMapping, Notifications, Groups, GroupsCount, ComponentUtils) {
    ...
    $scope.joinGroup = function() {
        if (!$scope.tree.currentNode) {
            Notifications.error('请选择要添加的组');
            return;
        }
        if (isMember($scope.tree.currentNode)) {
            Notifications.error('已添加组');
            return;
        }
...
module.controller('LDAPUserStorageCtrl', function($scope, $location, Notifications, $route, Dialog, realm,
    ...
    $scope.testAuthentication = function() {
        ...
        RealmLDAPConnectionTester.save({realm: realm.realm}, initConnectionTest("testAuthentication", $scope.instance.config), function() {
            Notifications.success("LDAP身份验证成功");
        }, function() {
            Notifications.error("LDAP身份验证失败");
        });
    }
...
module.controller('LDAPTabCtrl', function(Dialog, $scope, Current, Notifications, $location) {
    $scope.removeUserFederation = function() {
        Dialog.confirmDelete($scope.instance.name, 'ldap提供程序', function() {
...
module.controller('LDAPMapperCtrl', function($scope, $route, realm,  provider, mapperTypes, mapper, clients, Components, LDAPMapperSync, Notifications, Dialog, $location) {
    ...
    $scope.remove = function() {
        Dialog.confirmDelete($scope.mapper.name, 'ldap映射器', function() {
            Components.remove({
                realm : realm.realm,
                componentId : mapper.id
            }, function() {
    ...
    function triggerMapperSync(direction) {
        LDAPMapperSync.save({ direction: direction, realm: realm.realm, parentId: provider.id, mapperId : $scope.mapper.id }, {}, function(syncResult) {
            Notifications.success("数据同步成功。 " + syncResult.status);
        }, function(error) {
...
module.controller('LDAPMapperCreateCtrl', function($scope, realm, provider, mapperTypes, clients, Components, Notifications, Dialog, $location) {
    ...
    $scope.save = function() {
        if ($scope.mapperType == null) {
            Notifications.error("您需要选择映射器类型!");
            return;
        }
