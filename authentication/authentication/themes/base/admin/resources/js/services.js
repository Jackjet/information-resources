...
module.service('Dialog', function($modal) {
    ...
    dialog.confirmDelete = function(name, type, success) {
        var title = '删除 ' + escapeHtml(type.charAt(0).toUpperCase() + type.slice(1));
        var msg = '确定要永久删除' + type + ' ' + name + '?';
        var btns = {
            ok: {
                label: '删除',
...
module.factory('Notifications', function($rootScope, $timeout) {
    ...
    notifications.error = function(message) {
        notifications.message("danger", "", message);
    };

    notifications.warn = function(message) {
        notifications.message("warning", "", message);
    };
