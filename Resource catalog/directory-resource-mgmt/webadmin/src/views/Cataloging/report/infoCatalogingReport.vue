      ...
      <el-col ref="contentDomParents" :xl="20" :lg="19" class="tab_area">
        ...
        <div class="operate_area">
          <div class="related_btns">
            <el-row :gutter="20">
              <el-col :span="3">
                <el-button v-permission="['GET /admin/cataInfoReport/generateReport']" :loading="downloadBtn" class="add" size="mini" @click="exportFile">
                  <svg-icon icon-class="download" style="margin-right: 5px" />生成并下载报告
                </el-button>
              </el-col>
              <el-col :span="21">
                <el-button v-permission="['GET /admin/cataInfoReport/list']" class="remove fr" style="margin-left: 22px" icon="el-icon-search" @click="getTableData">查询</el-button>
              ...
              <template slot-scope="scope">
                <el-tooltip content="删除" effect="light" :open-delay="1000" placement="bottom">
                  <span class="tr_del_icon fl">
                    <svg-icon v-permission="['GET /admin/cataInfoReport/delete']" icon-class="tr_del" class-name @click="delRelated(scope.row)" />
                  </span>
                </el-tooltip>
              </template>
            </el-table-column>
...
export default {
  ...
  methods: {
    ...
    confirmDelete() {
      delResItem({ id: this.tempDelData.id })
        .then(res => {
          if (res.data.errno === 0) {
