        ...
        <div class="operate_area">
          <div class="related_btns" style="padding-bottom: 0">
            ...
            <el-row :gutter="20" class="headItemContent">
              <el-col>
                <el-button v-permission="['POST /admin/cataInfoApprove/approve2']" class="add headItem" size="mini" icon="el-icon-check" @click="addMenu(1)">终审通过</el-button>
                <el-button v-permission="['POST /admin/cataInfoApprove/approve2']" class="add headItem" size="mini" icon="el-icon-circle-close" @click="addMenu(2)">终审不通过</el-button>
                <el-button v-permission="['POST /admin/cataInfoApprove/approve2']" class="add headItem" size="mini" icon="el-icon-check" @click="addMenu(3)">全部终审通过</el-button>
                <el-button v-permission="['POST /admin/cataInfoApprove/approve2']" class="add headItem" size="mini" icon="el-icon-circle-close" @click="addMenu(4)">全部终审不通过</el-button>
              </el-col>
            </el-row>
          </div>
  ...
  methods: {
    ...
    addMenuDetail() {
      this.$refs.addbusinessForm.validate(valid => {
        if (valid) {
          ...
          cataShenHeApprove2(this.addbusinessForm.infoIds, asd)
            .then(res => {
    ...
    addMenu(num) {
      ...
      this.selecetCata.forEach(element => {
        if (element.audit_status == 5 || element.audit_status == 4) {
        } else {
