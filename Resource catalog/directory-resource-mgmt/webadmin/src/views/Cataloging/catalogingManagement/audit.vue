        ...
        <el-row class="tree_content_parent">
          <div class="tree_content">
            <el-scrollbar class="scrollbar_device">
              <el-tree v-loading="treeLoading" ref="treeDom" accordion :data="treeData" :props="defaultProps" highlight-current :expand-on-click-node="false" node-key="orgCd" :current-node-key="currentNodekey" :default-expanded-keys="expandedkeys" @node-click="handleNodeClick">
      ...
      <el-col ref="contentDomParents" :xl="20" :lg="19" class="tab_area">
          ...
          <div class="related_btns" style="padding-bottom: 0">
            <el-row :gutter="20">
              <el-col>
                ...
                <el-select v-model="listQuery.auditStatus" placeholder="审核状态" class="filter-item fl headItem" clearable @change="handleFilter">
                  <el-option v-for="item in saleTypes" :key="item.id" :label="item.name" :value="item.value" />
                  <template slot="prepend">所属部门</template>
                </el-select>
                <el-button v-permission="['GET /admin/cocataInfoApprove/cataInfoList']" class="remove fr" style="margin-left: 22px" icon="el-icon-search" @click="getTableData">查询</el-button>
              </el-col>
            </el-row>
            <el-row :gutter="20" class="headItemContent">
              <el-col>
                <el-button v-permission="['POST /admin/cataInfoApprove/approve']" class="add headItem" size="mini" icon="el-icon-check" @click="addMenu(1)">初审通过</el-button>
                <el-button v-permission="['POST /admin/cataInfoApprove/approve']" class="add headItem" size="mini" icon="el-icon-circle-close" @click="addMenu(2)">初审不通过</el-button>
                <el-button v-permission="['POST /admin/cataInfoApprove/approve']" class="add headItem" size="mini" icon="el-icon-check" @click="addMenu(3)">全部初审通过</el-button>
                <el-button v-permission="['POST /admin/cataInfoApprove/approve']" class="add headItem" size="mini" icon="el-icon-circle-close" @click="addMenu(4)">全部初审不通过</el-button>
              </el-col>
            </el-row>
          </div>
          <el-table ref="multipleTable" :data="tableData" stripe :header-cell-style="getRowClass" style="width: 100%;margin-top:20px" @selection-change="handleSelectionChange" v-loading="tableLoading">
            ...
            <el-table-column prop="update_dt" label="提交审核时间" show-overflow-tooltip />
            <el-table-column prop="audit_status" label="审核状态">
              <template slot-scope="scope">
                <div v-for="item in auditType" :key="item.id">
                  <span v-if="item.value==scope.row.audit_status">{{ item.name }}</span>
                </div>
              </template>
            </el-table-column>
        ...
        <el-dialog :modal-append-to-body="false" :title="dialogTitle" align="center" :visible.sync="addWorkOrder" width="48%" :close-on-click-modal="false" @close="closedDialog('addbusinessForm','addbusinessForm')">
          ...
          <div class="dialogDom" align="center">
            <el-button v-permission="['POST /admin/cataInfoApprove/approve']" size="mini" class="add" @click="addMenuDetail()">保 存</el-button>
...
export default {
  ...
  methods: {
    getByType() {
      dsysDictGetByTypet({ type: 'audit_status' })
        .then(res => {
          ...
          if (this.$route.query.status != null) {
            this.listQuery.auditStatus = this.$route.query.status
          }
          this.getTreeData()
        })
        .catch(response => {
      ...
      this.$refs.addbusinessForm.validate(valid => {
        if (valid) {
          ...
          this.selectOrgs.forEach(item => {
            infoIds.push(item.uview_id)
          })
          this.addbusinessForm.infoIds = infoIds
          var asd = {}
          asd.comment = this.addbusinessForm.comment
          asd.approveStatus = this.addbusinessForm.approveStatus
          cataShenHeApprove(this.addbusinessForm.infoIds, asd)
            .then(res => {
    ...
    addMenu(num) {
      ...
      this.selecetCata.forEach(element => {
        if (element.audit_status == 3 || element.audit_status == 2) {
        } else {
    ...
    getTreeData() {
      ...
      getDepartmentTree({ isTop: 0 })
        .then(response => {
          ...
          if (this.treeData[0] && this.treeData[0].children && this.treeData[0].children.length > 1) {//有多部门权限的
            this.currentNodekey = this.treeData[0].orgCd
            this.listQuery.pid = null
          } else {//只有本部门权限的
            this.currentNodekey = this.treeData[0].children[0].orgCd
            this.listQuery.pid = this.treeData[0].children[0].orgId
          }
    ...
    handleNodeClick(data, e) {
      ...
      if (this.listQuery.pid == 0) {
        if (this.treeData[0] && this.treeData[0].children && this.treeData[0].children.length > 1) {//多个部门权限
          this.listQuery.pid = null
        } else {
          this.listQuery.pid = this.treeData[0].children[0].orgId
        }
