<template>
  ...
  <el-table :data='tableData' stripe @expand-change="expandSelect" :expand-row-keys="expands" :header-cell-style="headerStyle" :cell-style="cellStyle" empty-text='暂无数据' class='el_tab_alage' :row-key='getRowKeys' @selection-change="handleSelectionChange">
    ...
    <template v-for="(index, item) in tableLabel">
      <el-table-column fit :align='index.textAlign' :key='item' :sortable='index.sort' v-if="index.type !== 'expand'" :label="index.label" :width="index.width" :show-overflow-tooltip="false" :prop="index.list">
        <template slot-scope="scope">
          <!-- 图片 -->
          ...
          <!-- 划过提示 -->
          <template v-else-if="index.type === 'hover'">
            <el-popover trigger="hover" placement="top">
              <p>{{ index.hoverText(scope.row).tips }}</p>
              <div slot="reference" class="name-wrapper">
                <el-tag size="medium">{{ index.hoverText(scope.row).text }}</el-tag>
              </div>
            </el-popover>
          </template>
          <!-- 内容自定义 -->
