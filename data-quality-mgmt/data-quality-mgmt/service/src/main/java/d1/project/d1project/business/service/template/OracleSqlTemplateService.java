 ...
 * 所有表名、字段都需要添加“”
 */
public class OracleSqlTemplateService extends SqlTemplateService {
    ...
    public String getUniqueSql(MetadataDataModel model) throws DoValidException {
        //SELECT name,age,sex FROM my_user GROUP BY name,age,sex HAVING count(*)>1;
    ...
    public String getContentSpecificationSql(MetadataDataModel model) throws DoValidException {
        //SELECT * FROM "my_user" WHERE NOT REGEXP_LIKE("name", '^[A-H]', 'i') OR "name" IS NULL;
        ...
        StringBuffer sql = new StringBuffer("SELECT * FROM \"" + tableName + "\" WHERE NOT REGEXP_LIKE(\"" + tableField + "\", '" + regExp + "', 'i') OR \"" + tableField + "\" IS NULL;");
        return sql.toString();
    }
