...
import d1.project.d1project.common.service.EventBus;
...
import d1.project.d1project.system.entity.*;
import d1.project.d1project.system.mapper.WebAdminUserMapper;
import d1.project.d1project.system.model.MySignInResult;
...
public class WebAdminUserServiceImpl implements IWebAdminUserService {
    ...
    private String resourceSharingPath;
    ...
    public void insertAll(List<WebAdminUserExcelImport> model, HttpServletRequest request, Function<WebAdminUserEntity, Boolean> accountFunction) throws DoValidException {
        List<String> accountEmptys = new ArrayList<>();
        List<String> accountExists = new ArrayList<>();
        ...
        if (!accountEmptys.isEmpty() || !accountExists.isEmpty() || !organizationThrows.isEmpty() || !roleThrows.isEmpty() || !accountMatchesThrows.isEmpty() || !emailMatchesThrows.isEmpty() || !phoneMatchesThrows.isEmpty() || !nameThrows.isEmpty() || !ssoAccountThrows.isEmpty()) {
            LinkedHashMap<String, String> throwsMap = new LinkedHashMap<>();
            if (!accountExists.isEmpty()) {
                throw new DoValidException(String.join(",", accountExists) + "在系统中账号已存在");
            }
            if (!accountEmptys.isEmpty()) {
                throwsMap.put("账号错误", String.join(",", accountEmptys));
            }
    ...
    protected MySignInResult ssoLogin(HttpServletRequest request) throws DoValidException {
        ...
        if ("admin".equals(name)) {
            entity = webAdminUserDao.findByAccount(name).orElse(null);
        } else {
    ...
    @Override
    public List<WebAdminUserEntity> findAll() {
    ...
    private void syncOrganization() throws Exception {
        JSONArray jsonArray = get("/api/organization/findAllList").getJSONArray("data");

        List<OrganizationEntity> oldOrg = organizationService.findAllList();
        List<OrganizationEntity> newOrg = JSONObject.parseArray(jsonArray.toJSONString(), OrganizationEntity.class);

        for (OrganizationEntity entity : newOrg) {
            organizationService.save(entity);
        }

        for (OrganizationEntity entity : oldOrg) {
            if (!newOrg.stream().filter(m -> m.getId().equals(entity.getId())).findAny().isPresent()) {
                organizationService.deleteById(entity.getId());
            }
        }
    }

    private void syncRole(HttpServletRequest request) throws Exception {
        JSONArray jsonArray = get("/api/role/findAllList").getJSONArray("data");

        List<RoleEntity> oldList = roleService.findAllList();
        List<RoleEntity> newList = JSONObject.parseArray(jsonArray.toJSONString(), RoleEntity.class);

        for (RoleEntity entity : newList) {
            Optional<RoleEntity> roleEntityOptional = roleService.findByName(entity.getName());
            if (!roleEntityOptional.isPresent()) {
                roleService.save(entity, request);
            }
        }

        for (RoleEntity entity : oldList) {
            if (!newList.stream().filter(m -> m.getName().equals(entity.getName())).findAny().isPresent()) {
                roleService.deleteByName(entity.getName(), request);
            }
        }
    }

    private void syncUser(HttpServletRequest request) throws Exception {
        JSONArray jsonArray = get("/api/user/findAllList").getJSONArray("data");

        List<WebAdminUserEntity> oldList = webAdminUserDao.findAll();
        List<WebAdminUserEntity> userList = JSONObject.parseArray(jsonArray.toJSONString(), WebAdminUserEntity.class);

        for (WebAdminUserEntity user : userList) {
            ...
            if (webAdminUserDao.existsByAccount(account)) {
                ...
                webAdminUser.setAvatar(user.getAvatar());
                webAdminUser.setPinyin(user.getPinyin());
                webAdminUser.setName(user.getName());
                webAdminUser.setSex(user.getSex());
                webAdminUser.setEmail(user.getEmail());
                webAdminUser.setIdCard(user.getIdCard());
                webAdminUser.setOrganizationName(user.getOrganizationName());
                webAdminUser.setRoleName(user.getRoleName());
            } else {
                BeanUtils.copyProperties(user, webAdminUser);
            }

            //插入用户角色
            roleUserService.deleteByUserId(webAdminUser.getId());
            Optional<RoleEntity> roleEntityOptional = roleService.findByName(webAdminUser.getRoleName());
            if (roleEntityOptional.isPresent()) {
            ...
            //插入用户机构
            String orgLinkName = webAdminUser.getOrganizationName();
            organizationUserService.deleteByUserId(webAdminUser.getId());
            if (!StringUtils.isEmpty(orgLinkName)) {
                Optional<OrganizationEntity> organizationEntityOptional = organizationService.findByNameLink(orgLinkName);
                if (organizationEntityOptional.isPresent()) {
                    OrganizationEntity org = organizationEntityOptional.get();
                    organizationUserService.deleteByUserId(webAdminUser.getId());
                    OrganizationUserEntity model1 = new OrganizationUserEntity(webAdminUser.getId(), org.getId());
                    organizationUserService.insert(model1, request);
                }
            }
        ...
        for (WebAdminUserEntity entity : oldList) {
            if (!userList.stream().filter(m -> m.getAccount().equals(entity.getAccount())).findAny().isPresent()) {
    ...
    private JSONObject get(String url) throws Exception {
        ...
        //请求
        String result = httpService.get(resourceSharingPath + url, headers, params);
