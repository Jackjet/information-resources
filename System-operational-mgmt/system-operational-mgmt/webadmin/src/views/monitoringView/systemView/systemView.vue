<template>
  <el-main>
    ...
    <environmentAdd :modal-objjj='title' ref="add" @getList="getList">
    </environmentAdd>
  </el-main>
</template>

<script>
import { systemList } from '@/api/monitoringView/http'
import TableList from '@/components/table/TableList.vue'
import Pagination from '@/components/table/Pagination.vue'
import environmentAdd from '../environmentView/environmentAdd'
export default {
  ...
  data() {
    return {
      ...
      tableHeader: [
        ...
        {
          id: false, type: '', label: '系统', list: 'system'
        },
        {
          id: false, type: '', label: '数据资源', list: 'dataSource'
        },
        {
          id: false, type: '', label: '内存资源', list: 'memorySource'
        },
        {
          id: false, type: 'html', label: '运行状态', list: 'status', html: (row) => { return '正常' }
        }
      ],
      tableOpction: {},
  ...
  methods: {
    async getList() {
      ...
      try {
        const res = await systemList(data)
