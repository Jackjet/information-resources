<template>
  <el-drawer :title="modalObjjj" direction="rtl" v-loading="loading" :visible.sync="dialogFormVisible" :before-close="ClearTextConeten" custom-class="demo-drawer" size="45%" :wrapperClosable="false" ref="drawer">
    <el-form :model="ruleForm" :rules="rules" ref="ruleForm" v-loading="loading">
      <el-form-item prop="nodeId" label="节点名称:" :label-width="this.formLabelWidth" class="InpitWidth">
        <el-select clearable style="width: 100%;" v-model="ruleForm.nodeId" :disabled="ProhibitFromm" @change="nodeChange" placeholder="请选择节点名称">
          <el-option v-for="item in nodeList" :key="item.value" :label="item.name" :value="item.id">
          </el-option>
        </el-select>
      </el-form-item>
      <el-form-item prop="databaseId" label="数据库名称:" :label-width="this.formLabelWidth" class="InpitWidth">
        <el-select clearable style="width: 100%;" v-model="ruleForm.databaseId" :disabled="ProhibitFromm" @change="selectGet" placeholder="请选择数据库名称">
          <el-option v-for="item in databasesList" :key="item.value" :label="item.name" :value="item.id">
          </el-option>
        </el-select>
      </el-form-item>
      <el-form-item label="备份位置:" :label-width="this.formLabelWidth" class="InpitWidth">
        <el-input clearable :disabled="ProhibitFrommm" autocomplete="off" v-model="ruleForm.location"></el-input>
      </el-form-item>
...
import {
  ...
  databaseBackupFindAllDatabases,
  databaseBackupFindAllNodes,
...
export default {
  ...
  methods: {
    ...
    selectGet(vId) {
      ...
      that.objj = this.databasesList.find((item) => {
        if (item.id === vId) {
          if (item.type === "Postgresql") {
            that.ruleForm.location = "/var/lib/sql_backup/postgre/";
          }
          if (item.type === "Mysql") {
            that.ruleForm.location = "/var/lib/sql_backup/mysql/";
          }
        }
    ...
    async add(data) {
      ...
      that.loading = true;
      const response = await databaseBackupInsert(da);
      that.loading = false;
    ...
    async findNodes() {
      const res = await databaseBackupFindAllNodes();
    ...
    async getNodeList() {
      ...
      const res = await databaseBackupFindAllDatabases(data);
      res.data.data.forEach((item) => {
