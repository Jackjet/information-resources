...
<script>
import { installPathFindAll, installPathDelete } from '@/api/deploymentView/http'
import { findAllNodes, findAllServices } from '@/api/moduleMonitorView/http'
import TableList from '@/components/table/TableList.vue'
import Pagination from '@/components/table/Pagination.vue'
import installationPathAdd from './installationPathAdd'
export default {
  components: { TableList, Pagination, installationPathAdd },
  data() {
    return {
      tableData: [],
      nodeList: [],
      ...
      tableHeader: [
        ...
        {
          id: false, type: '', label: '操作系统', list: 'os'
        },
        {
          id: false, type: '', label: '安装路径', list: 'path'
        },
        {
  ...
  methods: {
    ...
    deleteData(row) {
      ...
      that.$confirm('此操作将永久删除数据, 是否继续?', '提示', {
        ...
        that.Loading = true
        const response = await installPathDelete({ id: row.id })
        that.Loading = false
