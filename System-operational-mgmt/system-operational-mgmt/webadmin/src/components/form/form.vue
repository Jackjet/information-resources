  ...
  <el-form style='flex: 1;' v-if="formMode && formMode.form" ref="form" :model="form" :rules="rules"
    ...
    <template v-for='(item, index) in formMode.form'>
      ...
      <el-form-item
        ...
        <div v-if="item.ps" style="position: absolute;left: -18px;cursor:pointer; " @click="tooltip">
          <i class="el-icon-warning" @click="tooltip"></i>
        </div>
        <el-input
          type="textarea"
          :rows="7"
    ...
    <el-dialog title=" 配置内容说明" append-to-body :visible.sync="isShow">
      <div>
        如果您使用改参数配置来创建实例，则参数格式实例如下：
        <el-input
          type="textarea"
          :rows="10"
          class='el-slider'
