...
function excute () {
  ...
  var where = '';
  where = where + "type = '0' and ";
  where = where + "deleted = '0' and ";
  if(startTime !== '') {
    where = where + "add_time > '" + startTime + "' and ";
  }
  var endTime = new Date().format('yyyy-MM-dd HH:mm:ss');
  where = where + "add_time <= '" + endTime + "';";
  sql = sql + where;  
  startTime = endTime;
...
function insertlogs (log) {
  ...
  var parameters = {
    ...
    operand: config.instance().other.operationLog.source + "-" + log.action,
    result: statusS,
    description: log.result,
    operator: log.account,
    operatorId: log.admin,
    createTime: new Date(log.add_time).format('yyyy-MM-dd HH:mm:ss')
  };
